...
- Operator polish added after the hardening checkpoint:
  ...
  - `Ctrl+Shift+C` now handles copy across Claude, Codex, System log, and eligible `DOM` text selections
...
- Copy-path follow-up fix after first live report:
  ...
  - copy precedence is now explicit:
    - `DOM` selection first
    - otherwise the last active terminal surface
  - the system-log path is now supported without regressing later Claude/Codex copy
- Remaining proof gap for that operator polish:
  - closed on 2026-04-15 after real desktop-session validation
...
- Final copy-behavior evidence from the release build:
  - `Ctrl+Shift+C` works in the Claude pane, Codex pane, and System log on the real desktop app
  ...
  - `Ctrl+Shift+C` now follows the verified precedence rule:
    - `DOM` selection first
    - otherwise the last active terminal surface
  - keeping a stale system-log selection visible no longer blocks later Claude/Codex copy after the user clicks back into those panes
  - targeted frontend coverage now protects six copy-selection cases, including the supported system-log path
- Safe copy-design rule for the next slice:
  - keep the explicit precedence rule intact
  - extend it only with tests first if additional copy surfaces are introduced later
