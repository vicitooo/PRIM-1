...
- `tests/agent-events-summary.test.py`
  - validates `scripts/agent-events-summary.py` fixture summaries for `pane_signal`, grouped `route_delivery`, overlap-only `dispatch_attempt`, `dispatch_template_warning`, `supervisor_heartbeat`, `supervisor_alert`, `request_ack`, `request_ack_timeout`, and failed sideband lifecycle events
