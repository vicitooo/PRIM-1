...
#[cfg(windows)]
const INPUT_TRANSPORT_DRAIN_TIMEOUT: Duration = Duration::from_secs(5);

#[cfg(windows)]
fn flush_file_buffers_with_timeout(
    ...
    let caller_thread = unsafe { OpenThread(THREAD_TERMINATE, 0, GetCurrentThreadId()) };
    if caller_thread.is_null() {
        return Err(std::io::Error::last_os_error())
            .context("failed to pin PTY input-drain thread");
    }
    let caller_thread = caller_thread as usize;
    let (completed_tx, completed_rx) = sync_channel(1);
    let timed_out = Arc::new(AtomicBool::new(false));
    let watchdog_timed_out = Arc::clone(&timed_out);
    let watchdog = match thread::Builder::new()
        .name("prim1-pty-input-drain-watchdog".into())
        .spawn(move || {
            if matches!(
                ...
                // The real thread handle targets the caller, which is blocked in
                // FlushFileBuffers. The writer handle itself remains owned by the
                // session and locked until this cancellation has completed.
                unsafe {
                    CancelSynchronousIo(caller_thread as windows_sys::Win32::Foundation::HANDLE);
                }
            }
            unsafe {
                CloseHandle(caller_thread as windows_sys::Win32::Foundation::HANDLE);
            }
        }) {
        ...
        Err(error) => {
            ...
            return Err(error).context("failed to start PTY input-drain watchdog");
        }
    };
    ...
    watchdog
        .join()
        .map_err(|_| anyhow!("PTY input-drain watchdog panicked"))?;

    if timed_out.load(Ordering::SeqCst) {
        return Err(anyhow!(
            "PTY input-transport drain timed out after {}ms",
            timeout.as_millis()
        ));
    }
    if let Some(error) = flush_error {
        return Err(error).context("failed to drain PTY input transport");
    }
    Ok(())
}
...
pub trait PtySession: Send + Sync {
    ...
    /// Waits until the PTY transport has consumed all previously written input.
    /// Bracketed-paste delivery uses this boundary before starting its measured
    /// compatibility delay. Implementations must fail closed when they cannot
    /// distinguish an accepted write from a drained transport.
    fn flush_input(&self) -> anyhow::Result<()> {
        Err(anyhow!(
            "PTY session does not support an input-transport drain"
        ))
    }
...
impl PtySession for ConcretePtySession {
    ...
    fn flush_input(&self) -> anyhow::Result<()> {
        #[cfg(windows)]
        {
            // The borrowed raw handle is owned by `writer`, which remains alive
            // for the complete `ConcretePtySession` lifetime. Holding its mutex
            // also prevents any direct lower-layer caller from interleaving a
            // write between the completed frame and this drain. Cancellation is
            // deliberately lock-free and may interrupt the synchronous flush.
            let _writer = self.writer.lock().expect("pty writer poisoned");
            flush_file_buffers_with_timeout(
                self.writer_handle as windows_sys::Win32::Foundation::HANDLE,
                INPUT_TRANSPORT_DRAIN_TIMEOUT,
            )
        }
        ...
        {
            Err(anyhow!(
                "PTY input-transport drain is unavailable on this platform"
            ))
        }
    }
...
mod windows_real_process_tests {
    ...
    #[test]
    fn input_transport_drain_timeout_cancels_a_real_unread_pipe_flush() {
        use std::{
            fs::File,
        ...
        let _reader = unsafe { File::from_raw_handle(read_handle) };
        let mut writer = unsafe { File::from_raw_handle(write_handle) };
        writer
            .write_all(b"unread transport-drain probe")
            .expect("write unread pipe probe");

        let started = Instant::now();
        let error =
            flush_file_buffers_with_timeout(writer.as_raw_handle(), Duration::from_millis(100))
                .expect_err("unread pipe drain must hit its watchdog deadline");

        assert!(
            error.to_string().contains("timed out after 100ms"),
            "unexpected drain timeout error: {error:#}"
        );
        assert!(
            started.elapsed() < Duration::from_secs(2),
            "unread pipe drain exceeded its bounded cancellation window"
        );
    }

    #[test]
    fn flush_input_drains_the_real_conpty_transport_without_claiming_child_read() {
        ...
        let spec = LaunchSpec {
            ...
            display_name: "flush-real-conpty-input".to_owned(),
        };
        let session = Arc::new(
            ConcretePtySession::spawn(
                &spec,
        ...
        .expect("ConPTY cursor query was not observed");
        session
            .send_input("\u{1b}[1;1Rdrain-boundary-probe")
            .expect("queue probe behind the ConPTY cursor handshake");

        assert!(
            !drain_control.release_path.exists(),
            "the child-read gate must remain closed before the transport drain"
        );
        session.flush_input().expect("drain ConPTY input transport");
        assert!(
            session
                .try_wait()
                .expect("poll helper after ConPTY drain")
                .is_none(),
            "transport drain must not terminate the unread PTY child"
        );

        drain_control.release();
        session
            .kill()
            .expect("explicitly terminate drained ConPTY session");
        poll_until(Duration::from_secs(4), || {
            session
                .job
                .live_process_ids()
        ...
        .expect("session job retained processes after drain proof cleanup");
    }
