...
- **Configurable workspace/agent-working-root.** Today the agent working root is derived from `project_root.parent()` and the Claude driver appends `PRIM-1` to that path. Open-source clones whose directory is not named `PRIM-1` will see misconfigured working dirs. A real workspace/root config is on the roadmap.
