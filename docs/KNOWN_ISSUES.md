...
- **Configurable workspace/agent-working-root.** Today the agent working root is derived from `project_root.parent()` and the Claude driver appends `CLI-master-wrapper` to that path. Open-source clones whose directory is not named `CLI-master-wrapper` will see misconfigured working dirs. A real workspace/root config is on the roadmap.
- **Semantic per-pane work-state event.** Today the wrapper emits raw `session_output` chunks and a coarse `lifecycle_state`. A first-class `session_work_state` event (`idle | thinking | tool_call | blocked | error_loop`) is on the roadmap; until it lands, consumers infer pane state from byte traffic plus the workarounds in this document.
