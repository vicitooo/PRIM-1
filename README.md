...
- **Fail-closed Codex prompts** — a bounded per-run classifier observes raw Codex output across arbitrary chunks, including output emitted before PTY installation. Measured blocking prompts are applied before Ready and remain blocked until explicit non-blocked output. A separate bounded DEC-2026 frame scanner recognizes Codex 0.147.0's committed clean input-row/footer/cursor frame, so the first route does not depend on incidental startup output; cross-frame, malformed, truncated, and overlong evidence stays unknown, and the clean-frame recognizer never clears a latched block. Synthetic delivery rejects an unobserved Codex state, while raw terminal input remains available.
