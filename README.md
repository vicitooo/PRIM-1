...
- **Fail-closed Grok startup** — every Grok run receives a fresh native session ID and remains `Starting` until that exact run emits the measured ordered startup frames: a completed `Starting session…` repaint, then a later full-screen interactive-composer repaint. No timer grants readiness; the optional non-modal telemetry banner is ignored, replacement runs cannot inherit progress, and raw terminal input remains available throughout startup.
