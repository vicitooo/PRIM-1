...
- **Fail-closed Grok startup** — every Grok run receives a fresh native session ID and remains `Starting` until its bounded current-screen projection first observes `Starting session…`, then observes a completed trusted repaint with that marker and the launcher gone, the interactive composer and both measured shortcut labels present, and bracketed paste enabled. The driver follows only Grok 1.0.0's measured cursor, erase, scroll, text, and mode subset—using standard decoded Unicode display widths—rather than requiring one full-screen paint spelling or embedding a general terminal emulator. No timer grants readiness; unmeasured controls fail closed, the optional non-modal telemetry banner is ignored, replacement runs cannot inherit progress, and raw terminal input remains available throughout startup.
