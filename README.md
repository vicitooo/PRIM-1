...
- **Fail-closed Grok startup** — every Grok run receives a fresh native session ID and remains `Starting` through launcher, telemetry-consent, and startup activity. Only three seconds of measured silence after Grok's own `Starting session…` marker admits that exact run; later turns use semantic markers rather than generic quiet, and raw terminal input remains available throughout startup.
