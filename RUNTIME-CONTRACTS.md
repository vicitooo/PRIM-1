...
- for Claude, Codex, and Grok, the Rust supervisor requires enabled paste mode before writing one complete bracketed-paste frame, performs a fail-closed Windows ConPTY input-pipe drain bounded to five seconds, waits a one-second compatibility interval measured against Claude Code 2.1.226, Codex 0.147.0, and Grok Build 1.0.0, revalidates the exact run and safe observed work state, then writes one Enter under the same permit; the drain proves only that the host PTY transport consumed queued input bytes, not that the final child read them, and tests cover embedded CR, LF, tabs, Unicode, blank lines, and trailing spaces from the backend request body through the bytes admitted to the PTY writer
- if the ConPTY input-pipe drain fails or reaches its five-second deadline after the complete frame write, the route reports the full written-frame byte count as potentially partial and sends no Enter; the one-second interval begins only after a successful drain
