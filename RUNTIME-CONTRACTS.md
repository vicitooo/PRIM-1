...
- for Claude, Codex, and Grok, the Rust supervisor requires enabled paste mode before writing one complete bracketed-paste frame, performs a fail-closed Windows ConPTY input-pipe drain bounded to five seconds, waits a one-second base interval plus a proportional 500 milliseconds per MiB of framed input, revalidates the exact run and safe observed work state, then writes one Enter under the same permit; the exact final-child oracle observed approximately 10/19/174 milliseconds of parser lag at 1 KiB/64 KiB/1 MiB, while 500 milliseconds per MiB is the deliberately conservative stability margin, the 1 MiB body cap keeps that margin bounded to roughly half a second, the drain proves only host-transport consumption rather than final-child read, and tests cover embedded CR, LF, tabs, Unicode, blank lines, and trailing spaces from the backend request body through the bytes admitted to the PTY writer
- if the ConPTY input-pipe drain fails or reaches its five-second deadline after the complete frame write, the route reports the full written-frame byte count as potentially partial and sends no Enter; the composed base-plus-size interval begins only after a successful drain
