...
- for Claude and Codex, the Rust supervisor requires enabled paste mode before writing one complete bracketed-paste frame, waits a one-second compatibility interval measured against Claude Code 2.1.226 and Codex 0.147.0, revalidates the exact run and safe observed work state, then writes one Enter under the same permit; tests cover embedded CR, LF, tabs, Unicode, blank lines, and trailing spaces from the backend request body through the bytes admitted to the PTY writer
