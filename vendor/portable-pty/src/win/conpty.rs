...
impl MasterPty for ConPtyMasterPty {
    ...
    fn take_writer_with_raw_handle(
        &self,
    ) -> anyhow::Result<(Box<dyn std::io::Write + Send>, RawHandle)> {
        let writer = self
            .inner
            .lock()
            .unwrap()
            ...
            .take()
            .ok_or_else(|| anyhow::anyhow!("writer already taken"))?;
        let raw_handle = writer.as_raw_handle();
        Ok((Box::new(writer), raw_handle))
    }
}
...
impl SlavePty for ConPtySlavePty {
    ...
    fn spawn_command_in_job(
        ...
        let inner = self.inner.lock().unwrap();
        let child = inner.con.spawn_command_in_job(cmd, job as _)?;
        Ok(Box::new(child))
    }
}
