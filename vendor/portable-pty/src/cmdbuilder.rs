...
impl CommandBuilder {
    ...
    pub(crate) fn current_directory(&self) -> Option<Vec<u16>> {
        ...
        // Preserve an explicitly requested cwd even when it disappeared after
        // validation. CreateProcessW must fail that launch instead of silently
        // falling back to USERPROFILE and starting the child in the wrong
        // workspace.
        let cwd: Option<&OsStr> = self.cwd.as_deref();
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn explicit_missing_cwd_is_preserved_for_fail_closed_spawn() {
        let missing = r"C:\prim1-test-path-that-must-not-exist\missing";
        assert!(!Path::new(missing).exists());

        let mut cmd = CommandBuilder::new("dummy");
        cmd.cwd(missing);

        let expected = OsStr::new(missing)
            .encode_wide()
            .chain(std::iter::once(0))
            .collect::<Vec<_>>();
        assert_eq!(cmd.current_directory(), Some(expected));
    }
}
