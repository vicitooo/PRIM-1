...
pub trait MasterPty: Downcast + Send {
    ...
    /// Obtain the writable stream and its borrowed Windows handle so another
    /// thread can cancel a blocked synchronous write during shutdown.
    ///
    /// The handle is owned by the returned writer and remains valid only while
    /// that writer remains alive. Backends that cannot expose this pairing fail
    /// explicitly instead of returning an unrelated or short-lived handle.
    #[cfg(windows)]
    fn take_writer_with_raw_handle(
        &self,
    ) -> Result<
        (
    ...
    > {
        Err(anyhow::anyhow!(
            "this PTY backend does not expose an interruptible Windows writer"
        ))
    }
...
pub trait SlavePty {
    ...
    /// Spawns the command and assigns it to `job` as part of Windows process creation.
    ///
    /// The job handle must remain valid until this method returns. Backends that
    /// cannot provide create-time assignment fail rather than falling back to a
    /// post-spawn assignment with an escape window.
    #[cfg(windows)]
    fn spawn_command_in_job(
        &self,
        _cmd: CommandBuilder,
        _job: RawHandle,
    ) -> Result<Box<dyn Child + Send + Sync>, Error> {
        Err(anyhow::anyhow!(
            "this PTY backend does not support create-time Windows job assignment"
        ))
    }
}
