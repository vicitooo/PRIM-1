# PRIM-1 — Inter-Agent Handshake Instructions

Single source of truth for the first live Claude ↔ Codex handshake test under the supervisor.

Both panes load this file when the operator says "run the handshake test." Claude is the initiator. Codex is the responder. The operator watches the room.
...
- **Shared room** fans out to both panes plus the system log. Use `-To room -Scope room` whenever you want the operator to see an update. Treat the room as the operator's status feed.

### What incoming routed messages look like

- **Claude receives** a routed message as multiline input of the shape `\n[Direct message from codex]\n<content>\n` followed by an automatic Enter ~200ms later. The `[Direct message from <peer>]` prefix (or `[Room message from <peer>]` for room scope) is how you identify a routed message vs. a direct user input from the operator. Long routed messages may arrive as multiple part-labeled messages such as `[Room message from codex | part 1/3]`.
...
Treat incoming routed messages as if the operator had typed them into your pane. Respond normally, following the role spec below.

## 1. Sending a routed message

The pane's working directory is the configured workspace root: `<workspace-root>/`.

From your shell (Bash tool for Claude, shell exec for Codex), use the wrapped form below as the canonical pattern:

```bash
powershell -Command "& '<repo-root>/scripts/agent-route.ps1' -From <you> -To <peer|room> -Scope <direct|room> -Content '<single-quoted message>'"
```
...
```bash
# Claude -> Codex direct
powershell -Command "& '<repo-root>/scripts/agent-route.ps1' -From claude -To codex -Scope direct -Content 'hello from claude'"

# Codex -> Claude direct
powershell -Command "& '<repo-root>/scripts/agent-route.ps1' -From codex -To claude -Scope direct -Content 'hello from codex'"

# Either -> shared room (the operator's status feed)
powershell -Command "& '<repo-root>/scripts/agent-route.ps1' -From claude -To room -Scope room -Content 'status update for the operator'"
```
...
- **Inside the panes, use the wrapper script's absolute path.** The panes now start from `<workspace-root>/`, not from the wrapper root, so relative `.\scripts\...` paths are no longer reliable there.
...
powershell -Command "& '<repo-root>/scripts/control-plane.ps1' -Action input -Session claude -ContentFile '<repo-root>/.runtime/compact-prompts/compact-full.txt'"
powershell -Command "& '<repo-root>/scripts/agent-key.ps1' -Session claude -Key enter"

# Only the slash-command arguments live in the file
powershell -Command "& '<repo-root>/scripts/agent-slash.ps1' -Session claude -Slash compact -ArgsFile '<repo-root>/.runtime/compact-prompts/compact-args.txt'"
powershell -Command "& '<repo-root>/scripts/agent-key.ps1' -Session claude -Key enter"
```
...
When the operator says "run the handshake test":
...
1. **Generate a run token and timestamped target path.**
   ...
   ```bash
   powershell -Command "& '<repo-root>/scripts/new-smoke-token.ps1'"
   ```
...
2. **Announce to the operator via the canonical helper:**

   ```bash
   powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor claude -Action start -Token '<token>' -Path '<absolute_path>'"
   ```

3. **Dispatch Codex** with the canonical direct message builder. Do not hand-write the preamble.

   ```bash
   powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor claude -Action dispatch -Token '<token>' -Path '<absolute_path>'"
   ```
...
4. **Launch the external timeout watchdog** before ending your turn. Claude cannot self-tick while idle in the PTY, so timeout failure reporting must come from a separate process.
   ...
   ```bash
   powershell -Command "Start-Process powershell -WindowStyle Hidden -ArgumentList '-ExecutionPolicy','Bypass','-File','<repo-root>/scripts/handshake-watchdog.ps1','-Token','<token>'"
   ```
...
8. **Report the result via room:**
   ...
   - **Pass** — file exists and contents match:
     ```bash
     powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor claude -Action pass -Token '<token>' -Path '<absolute_path>'"
     ```
   - **Fail** — any mismatch, missing file, timeout, or error:
     ```bash
     powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor claude -Action fail -Token '<token>' -Reason '<one-line reason>'"
     ```

9. **Stop.** Do not retry. Do not clean up the file. Wait for the operator's next instruction.
...
4. **Reply to Claude** with the exact acknowledgement string Claude asked for:
   ...
   ```bash
   powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor codex -Action ready -Token '<token>'"
   ```

5. **Announce to the operator via room:**

   ```bash
   powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor codex -Action status -Token '<token>' -Path '<path>'"
   ```
...
```bash
powershell -Command "& '<repo-root>/scripts/handshake-route.ps1' -Actor codex -Action fail -Token '<token-if-known-or-UNKNOWN>' -Reason '<one-line reason>'"
```

## 4. Trigger prompts (for the operator to paste)

**Paste into Claude's pane first:**

> Read `<repo-root>/instructions.md`. Execute your role from section 2 (Claude's role — initiator). Begin now.

**Then paste into Codex's pane:**

> Read `<repo-root>/instructions.md`. Section 3 is your role (Codex — responder). Stand by for Claude's routed message and respond as instructed.
...
## 5. What success looks like (the operator's view)
...
- **Do not clean up.** Leave the temp file and partial state in place so the operator can inspect it.
- **Do not invent a fix.** If Claude thinks Codex misbehaved (wrong token, wrong path, timeout), Claude reports `HANDSHAKE FAIL` and stops. The operator debugs.
...
- the operator → Claude direct (the trigger)
